...
.EXAMPLE
    ...
    Tests Againts the defaultTimeServer used my the computer.
.OUTPUTS
...
#>
    ...
    Param (
        ...
        $defaultTimeServer = w32tm /query /source /computer:$computerName
        if ($useDefaultTimeServer) {
            $Server = $defaultTimeServer
        }    

        $ntpDataSet = get-ntpData -computerName $computerName -server $server

        foreach ( $ntpDataPacket in $ntpDataSet){
            
            $computerName = $ntpDataPacket.psComputerName
            $ntpData = $ntpDataPacket.ntpData
            $t1 = $ntpDataPacket.TransactionStart
            $t4 = $ntpDataPacket.TransactionEnd
...
function get-ntpData {
    ...
    $ipAddressOfNtpServer = Resolve-DnsName admdc03.admnet.oakland.edu | where-object {$_.section -eq "Answer"} | select -ExpandProperty ipAddress

    foreach ($computer in $computerName){
        invoke-command -ComputerName $computer -scriptblock {    
            param($ipAddressOfNtpServer)
        ...
        } -ArgumentList $ipAddressOfNtpServer
    }
}
