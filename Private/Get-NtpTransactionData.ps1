function Get-NtpTransactionData
{
    <#
.SYNOPSIS
     Get NTP Tansactio Data, including Ntp raw binary data and transaction start and end time.
.DESCRIPTION
...
.EXAMPLE
    Get-NtpTransactionData

    Get Ntp transaction dtat for localhost
.EXAMPLE
    Get-NtpTransactionData -computerName pc1 -server ntp.org
.OUTPUTS
...
#>
    ...
    try
    {
        $ipAddressOfNtpServer = Resolve-DnsName $server | Where-Object { $_.section -eq "Answer" } | Select-Object -ExpandProperty ipAddress | Select-Object -first 1
    }
    catch
    {
        Write-Warning "Unable to perform a reverse lookup of $server"
    }
